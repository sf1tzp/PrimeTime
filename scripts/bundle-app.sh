...
# --arch builds land in .build/<arch>-apple-macosx/release, not .build/release.
REL="$ROOT/.build/${ARCHS%% *}-apple-macosx/release"
...
lipo_bin() {
    local slices=()
    for ARCH in $ARCHS; do
        slices+=("$ROOT/.build/$ARCH-apple-macosx/release/$1")
    done
