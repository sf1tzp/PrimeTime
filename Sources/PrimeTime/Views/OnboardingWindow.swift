...
/// → starter label sets — shown once on a fresh install and on every demo
/// launch (the demo defaults suite is wiped per launch, so the flag is always
/// unset there).
///
/// Set PRIMETIME_ONBOARDING=1 to force it regardless of the flag, for
...
final class OnboardingWindowManager: NSObject {
    ...
    func showIfNeeded(model: AppModel) {
        let forced = ProcessInfo.processInfo.environment["PRIMETIME_ONBOARDING"] == "1"
