...
func TestGQL_CreateLabelSet_succeeds(t *testing.T) {
	...
	set, err := resolver.CreateLabelSet(fake.User(5), "Deep Work", "brain.head.profile", labels("type", "programming", "focus", "deep"))
...
func TestGQL_CreateLabelSet_fails_emptyName(t *testing.T) {
	...
	resolver := ResolverForLabelSet{DB: db.DB}
	_, err := resolver.CreateLabelSet(fake.User(5), "  ", "tag", labels())
...
func TestGQL_LabelSets_ordered_andScopedToUser(t *testing.T) {
	...
	resolver := ResolverForLabelSet{DB: db.DB}
	_, err := resolver.CreateLabelSet(fake.User(5), "First", "1.circle", labels("a", "1"))
	require.Nil(t, err)
	_, err = resolver.CreateLabelSet(fake.User(5), "Second", "2.circle", labels())
	require.Nil(t, err)
	_, err = resolver.CreateLabelSet(fake.User(4), "Other", "tag", labels())
	require.Nil(t, err)
...
func TestGQL_UpdateLabelSet_replacesMembers(t *testing.T) {
	...
	resolver := ResolverForLabelSet{DB: db.DB}
	set, err := resolver.CreateLabelSet(fake.User(5), "Meeting", "person.2", labels("type", "meeting"))
	require.Nil(t, err)

	updated, err := resolver.UpdateLabelSet(fake.User(5), set.ID, "Standup", "person.3", labels("type", "meeting", "kind", "standup"))
	require.Nil(t, err)
	require.Equal(t, &gqlmodel.LabelSet{
		ID:         set.ID,
		Name:       "Standup",
...
func TestGQL_UpdateLabelSet_fails_otherUsersSet(t *testing.T) {
	...
	resolver := ResolverForLabelSet{DB: db.DB}
	set, err := resolver.CreateLabelSet(fake.User(4), "Other", "tag", labels())
	require.Nil(t, err)

	_, err = resolver.UpdateLabelSet(fake.User(5), set.ID, "Stolen", "tag", labels())
	require.Error(t, err)
}
...
func TestGQL_MoveLabelSet_reorders(t *testing.T) {
	...
	resolver := ResolverForLabelSet{DB: db.DB}
	a, _ := resolver.CreateLabelSet(fake.User(5), "A", "tag", labels())
	_, err := resolver.CreateLabelSet(fake.User(5), "B", "tag", labels())
	require.Nil(t, err)
	_, err = resolver.CreateLabelSet(fake.User(5), "C", "tag", labels())
	require.Nil(t, err)
